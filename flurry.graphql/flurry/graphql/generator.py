...
class GraphqlGenerator:
    ...
    gql_types: Dict[Type[_GraphqlAnyObject], Dict[str, Type[_GraphqlAnyObject]]]
    ...
    def field_to_graphql_field(
        ...
        containing_type: Type[_GraphqlAnyObject],
    ) -> graphene.Field:
    ...
    def schema_to_graphql_object(
        self,
        name: str,
        the_schema: Mapping[str, schema.Field[Any]],
        object_type: Type[_GraphqlAnyObject] = graphene.ObjectType,
    ...
    def generate_graphql_query(
        ...
        if len(query.__schema__) > 0:
            resolver_args = {
                name: self.field_to_graphql_argument(field)
                for name, field in query.__schema__.items()
            }

        async def resolver_fn(_root: Any, info: graphene.ResolveInfo, **kwargs):
            try:
                query_inst = query(**kwargs)
                result = query_inst.fetch(info.context.context)
                if isawaitable(result):
                    result = await result
                return result
            except Exception as err:
    ...
    def collect_graphql_object_types(self):
        ...
        all_types: List[Type[_GraphqlAnyObject]] = []
